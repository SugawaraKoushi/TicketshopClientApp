...
const FoundFlights = () => {
    ...
    const fetchData = async () => {
        try {
            const response = await axios.get("http://localhost:8080/flight/getWhenDate", { params: state });
    ...
    const generateLuggagePrice = (size) => {
        ...
        for (let i = 0; i < size; i++) {
            const price = Math.random() * (max - min) + min;
            setLuggagePrices([...luggagePrices, price.toFixed(0)]);
        }
    }
                ...
                <Container fluid sx={{ margin: "auto", width: "45%", padding: "20px" }}>
                    <Box sx={{ display: "flex", width: "100%", border: "1px solid", borderColor: "rgb(217, 217, 217)", borderRadius: "15px" }} id="flight-side-content">
                        <Box sx={{ padding: "5px", display: "flex", flexDirection: "column", alignContent: "space-between", width: "25%", height: "150px", borderRight: "1px solid", borderColor: "rgb(217, 217, 217)" }}>
                            <InputLabel id="price-label" sx={{ textAlign: "center", fontSize: "18pt" }}>{prices[i]} ₽</InputLabel>
                            <FormControlLabel
                        ...
                        <Grid container sx={{ width: "75%", padding: "5px", alignContent: "center" }}>
                            <Grid item xs={2}>
                                <InputLabel sx={{ fontSize: "18pt", textAlign: "center" }}>
                                    {new Date(flight.departureDate).getHours()}:{new Date(flight.departureDate).getMinutes()}
                                </InputLabel>
                                <InputLabel sx={{ fontSize: "8pt", textAlign: "left" }}>
                                    {flight.from.city.name}<br />1 янв, пн
                                </InputLabel>
                            </Grid>
                            <Grid item xs={8} sx={{ borderBottom: "solid", borderColor: "rgb(217, 217, 217)", height: "25%" }}>
                                <Box sx={{ display: "flex", justifyContent: "space-between" }}>
                                    <FlightTakeoffIcon color="disabled" />
                                    <InputLabel sx={{ color: "rgb(217, 217, 217)" }}>
                                        В пути {calculateTimeDifference(flight.departureDate, flight.arrivingDate)} часа
                                    </InputLabel>
                            ...
                            <Grid item xs={2}>
                                <InputLabel sx={{ fontSize: "18pt", textAlign: "center" }}>
                                    {new Date(flight.arrivingDate).getHours()}:{new Date(flight.arrivingDate).getMinutes()}
                                </InputLabel>
                                <InputLabel sx={{ fontSize: "8pt", textAlign: "right" }}>
                                    {flight.to.city.name}<br />1 янв, пн
                                </InputLabel>
                            </Grid>
                        </Grid>
                    </Box>
